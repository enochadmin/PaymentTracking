...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
        ...
        <form action="{{ route('payment-documents.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="supplier_id" name="supplier_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" {{ old('supplier_id') == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }} ({{ $supplier->supplier_type }})</option>
                        @endforeach
                    </select>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="reason_for_payment" name="reason_for_payment" rows="3" class="mt-1 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>{{ old('reason_for_payment') }}</textarea>
                    @error('reason_for_payment') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    @error('payment_type') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    @error('cost_type') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="amount" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Amount</label>
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="number" name="amount" id="amount" step="0.01" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-3 pr-12 sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="0.00" value="{{ old('amount') }}" required>
                ...
                <div>
                    ...
                    <input type="date" name="received_date" id="received_date" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ old('received_date') }}" required>
                    @error('received_date') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="submission_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Submission Date</label>
                    <input type="date" name="submission_date" id="submission_date" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white" value="{{ old('submission_date', date('Y-m-d')) }}" required>
                    ...
                    <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="file" name="delivery_note" id="delivery_note" class="mt-1 block w-full text-sm text-gray-500
                                ...
                                file:bg-indigo-50 file:text-indigo-700
                                hover:file:bg-indigo-100">
                            @error('delivery_note') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>
            </div>
